...
class mainApp(QMainWindow, Ui_MainWindow):
    ...
    def DataBase(self):
        ...
        self.db1.execute(
            'CREATE TABLE IF NOT EXISTS TEST(ui TEXT primary key,name TEXT,size INT,ts TEXT,savep ,te TEXT,stat TEXT)')
    ...
    ###################################################### ui change #######################################################
    def Handel_Ui(self):
        self.setWindowIcon(QIcon('cv.ico'))  ##icon program
    ...
    def onstart(self):  # To Run The Thread And Make Condition For The Line Edit If It Empty
        try:
            ...
            self.calc.count_changed.connect(
                self.count_change_process)  # To Connect The Signal From Thread To The Funciton In The Main Window
            self.calc.duration.connect(
                self.count_change_process_Single_label)  # To Connect The Signal From Thread To The Funciton In The Main Window
            self.calc.finsish.connect(
                self.download_finshed_Single)  # To Connect The Signal From Thread To The Funciton In The Main Window
            self.calc.error.connect(
                self.Handel_Error_Single)  # To Connect The Signal From Thread To The Funciton In The Main Window
            ...
            if self.calc.url_d == '':
                ...
                if save == '':
                    ...
                    for i in p:
                        o.append(i[0])
                    if self.ui in o:
                        self.db1.execute('UPDATE TEST SET name=? ,ts=?,savep=?,stat=? WHERE ui=?',
                                         (save1, time1, save, 'جاري التحميل', self.ui))
                        self.db.commit()
                        print('don')
                        self.db1.close()
                        self.db.close()
                        ...
                        self.db1.execute('INSERT INTO TEST(ui,name,ts,savep,stat ) VALUES(?,?,?,?,?)',
                                         (self.ui, save1, time1, save, 'جاري التحميل'))
                        self.db.commit()
                        self.db1.close()
                        self.db.close()
    ...
    ########################################################################################################################
    ################################## This Section For Get Data From Threading About Quality And put It In ComboBox #######
        ...
        try:
            ...
            if self.video_download.video_url == '':
                ...
                if self.comboBox.currentIndex() == (-1):
                    return self.label_2.setText("اختار جودة الفديو")
                else:
                    self.label_2.setText(" ")
                    if self.video_download.save_video == '':
                        ...
                        if self.ul in o:
                            print(12)
                            self.db1.execute('UPDATE TEST SET name=? ,ts=?,savep=?,stat=? WHERE ui=?',
                                             (save1, time1, save, 'جاري التحميل', self.ul))
                            self.db.commit()
                            self.db1.close()
                            self.db.close()
                        else:
...
class Thread_run(QThread):
    ...
    def run(self):
        try:
            # urllib.request.build_opener(h)
            urllib._urlopener = AppURLopener()
            # urllib.request.urlretrieve(self.url_d,self.save_d,self.Handel_ProgressBar)
            urllib._urlopener.retrieve(self.url_d, self.save_d, self.Handel_ProgressBar)
        except Exception as a:
            print(a)
    ...
    def Handel_ProgressBar(self, block_num, blocksize, totalsize):
        ...
        try:
            if totalsize > 0:
                ...
                self.count_changed.emit(int(percent))
                tt = round((totalsize / (1024 * 1024)), 2)
                d = round((downloaded / (1024 * 1024)), 2)
                rf = '''  نسبة التحميل:{2}%    تم التحميل :{1}MB   الحجم الكلي : {0} '''.format(tt, d, int(percent))
                ay = [rf, tt]
                self.duration.emit(ay)
                # http://51.75.53.11/Oceanofgames.com/Counter_Strike_1.6_War_Space.zip?md5=7qZM2bqX9IohWdBeoIkcnw&expires=1575483409
    ...
    def run(self):
        try:
            ...
            for stream in video_streams:
                size = round((stream.get_filesize() / (1024 * 1024)), 2)
...
class Thread_Download_Video(QThread):
    ...
    def run(self):
        try:
            ...
            download = video_stream[self.indexC].download(filepath=self.save_video,
                                                          callback=self.Video_Single_ProgressBar)
        except Exception as a:
            self.erro.emit()
            print(a)
    ...
    def Video_Single_ProgressBar(self, total, recived, ratio, rate, time):
        try:
            ...
            if total > 0:
                pro = ratio * 100
                Sze = round((total / (1024 * 1024)), 2)
                Recvd = round((recived / (1024 * 1024)), 2)
                Rate = round((rate / (1024 * 1024)), 2)
                self.count_change_Prog.emit(int(pro))
    ...
    def Playlist_Progress(self, total, recived, ratio, rate, time):
        try:
            ...
            if total > 0:
                ...
                Recvd = recived
                Rate = round((rate / (1024 * 1024)), 2)
                self.count_progress.emit(int(pro))
...
if __name__ == "__main__":
    main()
# https://www.youtube.com/watch?v=sA0-QXbLnaE    https://www.youtube.com/playlist?list=PLY_6uAtgkYXmXFl-zzZ0wQErGi4AbgSkn    https://www.youtube.com/playlist?list=PLppdDIobDRH1flckH2-e40zdsZbCGCMJn     https://www.youtube.com/watch?v=FNeyXUQYatI
